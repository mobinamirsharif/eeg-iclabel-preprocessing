...
# Increased from 10 -> 15 so ICA isn't forced to cram 32 channels
# of variance into too few components (which was likely pushing
# leftover/mixed variance into diffuse "heart beat"-looking maps)
N_COMPONENTS = 15

SAMPLING_RATE = 128.0
LOW_FREQ = 1.0
HIGH_FREQ = 55.0  # kept from v2 (near Nyquist=64Hz)

# How many CONSECUTIVE trials to use, concatenated end-to-end
N_TRIALS_FOR_ICA = 5

BASE_OUTPUT_DIR = "ICLabel_DEAP_Results_CORRECTED_V3"
...
print(f"ICA components: {N_COMPONENTS} (increased from 10)")
print(f"Trials per subject for ICA: {N_TRIALS_FOR_ICA} (concatenated with 0.5s crossfade)")
print("=" * 80)


# ============================================================
# DATASET PATH (Direct Local Cache Search - No 403 Error)
# ============================================================
...
if not data_dir:
    try:
        ...
        print(f"KaggleHub online download skipped: {e}")
...
for subject_idx in range(1, 33):
    ...
    try:
        ...
        # ------------------------------------------------
        # Linear crossfade ramp (~0.5s) between trials
        # ------------------------------------------------
        ramp_samples = int(0.5 * SAMPLING_RATE)
        ...
        # ------------------------------------------------
        # Extended Infomax ICA (15 Components)
        # ------------------------------------------------
        ica = ICA(
        ...
        plt.title(
            f"DEAP Subject {sub_id_str.upper()} (Electrode Fp1) - Raw vs. Artifact-Reduced EEG (V3)",
        ...
        subject_json_report = {
            ...
            "trial_boundary_crossfade_seconds": 0.5,
...
global_summary_json = {
    "experiment_metadata": {
        ...
        "trial_boundary_handling": "0.5s linear crossfade between concatenated trials",
...
print("\n" + "=" * 80)
print("BATCH PROCESSING COMPLETED (V3: MORE COMPONENTS + CROSSFADED TRIALS + TOPO PLOTS)")
print("=" * 80)
...
print(f"ICA components used: {N_COMPONENTS}")
