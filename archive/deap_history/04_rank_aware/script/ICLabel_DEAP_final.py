...
SAMPLING_RATE = 128.0
LOW_FREQ = 1.0
HIGH_FREQ = 55.0  # near Nyquist (64 Hz) given the 128 Hz sampling rate

N_TRIALS_FOR_ICA = 5
CROSSFADE_SECONDS = 0.5

BASE_OUTPUT_DIR = "ICLabel_DEAP_Results_FINAL"

DIR_CLEANED = os.path.join(BASE_OUTPUT_DIR, "cleaned_data")
DIR_FIGURES = os.path.join(BASE_OUTPUT_DIR, "figures")
DIR_TOPO = os.path.join(BASE_OUTPUT_DIR, "heartbeat_topographies")
...
for d in [DIR_CLEANED, DIR_FIGURES, DIR_TOPO, DIR_REPORTS, DIR_SUMMARY, DIR_BENCHMARK]:
...
print("ICA component count: RANK-AWARE (computed per subject, not fixed)")
print("=" * 80)


print("\nResolving DEAP dataset path via kagglehub...")
...
for subject_idx in range(1, 33):
    ...
    try:
        ...
        n_trials_for_ica = min(N_TRIALS_FOR_ICA, total_trials)

        # ------------------------------------------------
        # Continuous EEG with crossfaded trial boundaries
        # ------------------------------------------------
        ramp_samples = int(CROSSFADE_SECONDS * SAMPLING_RATE)
        trial_list = [eeg_channels_only[t] for t in range(n_trials_for_ica)]

        continuous_eeg = trial_list[0].copy()
        for nxt in trial_list[1:]:
            if ramp_samples > 0 and continuous_eeg.shape[1] >= ramp_samples and nxt.shape[1] >= ramp_samples:
                fade_out = np.linspace(1, 0, ramp_samples)
                fade_in = np.linspace(0, 1, ramp_samples)
                tail = continuous_eeg[:, -ramp_samples:] * fade_out
                head = nxt[:, :ramp_samples] * fade_in
                blended = tail + head
                continuous_eeg = np.concatenate(
                    [continuous_eeg[:, :-ramp_samples], blended, nxt[:, ramp_samples:]],
                    axis=1
                )
            else:
                continuous_eeg = np.concatenate([continuous_eeg, nxt], axis=1)
        ...
        # RANK-AWARE ICA
        #
        # The DEAP "data_preprocessed_python" files have already
        # had EOG components removed via ICA by the original DEAP
        # authors, and we just applied a common-average reference
        # on top. Both operations reduce the true rank of the data
        # below 32. Asking ICA for more components than the actual
        # rank supports forces it to fabricate components with no
        # real physical source -- these tend to be diffuse, low
        # structure maps that ICLabel systematically mislabels as
        # "heart beat". We compute the actual rank and only ask
        # ICA for that many components (minus 1, as a safety
        # margin for the average-reference rank deficiency).
        # ------------------------------------------------
        rank_info = mne.compute_rank(raw_mne, tol="auto", verbose=False)
        eeg_rank = rank_info["eeg"]
        n_components_safe = max(2, eeg_rank - 1)

        print(f"[{sub_id_str}] Computed data rank: {eeg_rank} -> using {n_components_safe} ICA components")
        ...
        n_components_actual = len(predicted_labels)
        ...
        for comp_idx, (assigned_label, prob_val) in enumerate(
            ...
            if assigned_label == "heart beat":
                ...
                if topo_plots_saved < MAX_TOPO_PLOTS_TOTAL:
                    try:
                        fig = ica.plot_properties(
                            raw_mne, picks=[comp_idx], show=False, verbose=False
                        )[0]
                        topo_fig_path = os.path.join(
                            DIR_TOPO,
                            f"{sub_id_str}_ic{comp_idx:02d}_heartbeat_p{prob_float:.2f}.png"
                        )
                        fig.savefig(topo_fig_path, dpi=150)
                        plt.close(fig)
                        topo_plots_saved += 1
                    except Exception as plot_err:
                        print(f"    [warn] could not save topo plot for IC{comp_idx}: {plot_err}")

            is_excluded = assigned_label in ARTIFACT_CLASSES
            if is_excluded:
                excluded_component_indices.append(comp_idx)
                exclusion_reason = f"Classified as {assigned_label} with probability {prob_float:.4f}"
            else:
        ...
        print(
            ...
            f"{len(excluded_component_indices)}/{n_components_actual}"
        )
        ...
        cleaned_fif_path = os.path.join(DIR_CLEANED, f"deap_{sub_id_str}_cleaned_eeg.fif")
        ...
        plt.title(
            f"DEAP Subject {sub_id_str.upper()} (Electrode Fp1) - Raw vs. Artifact-Reduced EEG",
        ...
        subject_json_report = {
            ...
            "trial_boundary_crossfade_seconds": CROSSFADE_SECONDS,
            ...
            "computed_data_rank": int(eeg_rank),
            "ica_components_used": int(n_components_actual),
            "rank_aware_component_selection": True,
            "excluded_components_count": len(excluded_component_indices),
            "excluded_components_percentage": float(
                round((len(excluded_component_indices) / n_components_actual) * 100, 2)
            ),
        ...
        global_subject_summaries.append({
            ...
            "ica_components": n_components_actual,
...
if not df_heart_probe.empty:
    df_heart_probe.to_csv(os.path.join(DIR_SUMMARY, "heart_beat_confidence_probe.csv"), index=False)
...
aggregate_exclusion_rate = (total_excluded_components / max(1, total_ica_components)) * 100
...
global_summary_json = {
    "experiment_metadata": {
        ...
        "ica_component_selection": "rank-aware (computed per subject)",
        "trial_boundary_handling": f"{CROSSFADE_SECONDS}s linear crossfade between concatenated trials",
    ...
    "heart_beat_confidence_probe": {
        ...
        "topography_plots_saved": topo_plots_saved,
        "topography_plots_directory": DIR_TOPO
    },
    "hardware_benchmark_summary": {
        ...
        "average_subject_processing_time_seconds": (
            float(round(np.mean(processing_times_successful), 4)) if processing_times_successful else 0.0
        ),
        "minimum_subject_processing_time_seconds": (
            float(round(np.min(processing_times_successful), 4)) if processing_times_successful else 0.0
        ),
        "maximum_subject_processing_time_seconds": (
            float(round(np.max(processing_times_successful), 4)) if processing_times_successful else 0.0
        )
    },
...
# ============================================================
# FINAL SUMMARY FIGURE (bar chart + pie chart), saved into
# summary/ alongside the CSV/JSON -- same layout as the
# reference figure: per-subject stacked bars (total excluded
# vs heart-beat-specifically), plus an overall class pie chart.
# ============================================================

successful_df = df_summary[df_summary["status"] == "Success"].copy()
...
if not successful_df.empty:
    ...
    ax_bar = axes[0]
    subject_ids = successful_df["subject_id"].str.upper()
    total_excluded = successful_df["excluded_components"]
    heart_components = successful_df["heart_components"]

    x_positions = np.arange(len(subject_ids))
    ax_bar.bar(x_positions, total_excluded, color="#e8785a", label="Total Noise ICs Removed")
    ax_bar.bar(x_positions, heart_components, color="#b3382c", label="Heart Beat Artifacts")

    ax_bar.set_xticks(x_positions)
    ax_bar.set_xticklabels(subject_ids, rotation=90, fontsize=7)
    ax_bar.set_xlabel("Subject ID")
    ax_bar.set_ylabel("Number of Independent Components")
    ax_bar.set_title(
        f"Artifact Components Removed per Subject (DEAP {len(subject_ids)} Subjects)",
        fontsize=11, fontweight="bold"
    )
    ax_bar.legend(loc="upper right", fontsize=8)
    ax_bar.grid(axis="y", linestyle="--", alpha=0.4)

    # --- Right: overall class distribution pie chart ---
    ax_pie = axes[1]
    pie_labels = ["Brain", "Muscle artifact", "Eye blink", "Heart beat", "Other"]
    pie_values = [
        total_class_counts["brain"],
        total_class_counts["muscle artifact"],
        total_class_counts["eye blink"],
        total_class_counts["heart beat"],
        total_class_counts["line noise"] + total_class_counts["channel noise"] + total_class_counts["other"]
    ]
    pie_colors = ["#2ca02c", "#ff7f0e", "#f2c14e", "#d62728", "#1f77b4"]

    # Only plot non-zero slices to avoid clutter, but keep order
    nonzero = [(l, v, c) for l, v, c in zip(pie_labels, pie_values, pie_colors) if v > 0]
    labels_nz = [x[0] for x in nonzero]
    values_nz = [x[1] for x in nonzero]
    ...
    ax_pie.set_title(
        f"Overall Signal vs Artifact Distribution Across All {len(subject_ids)} Subjects",
        fontsize=11, fontweight="bold"
    )

    plt.tight_layout()
    summary_fig_path = os.path.join(DIR_SUMMARY, "artifact_distribution_summary.png")
    plt.savefig(summary_fig_path)
    plt.close(fig)
    print(f"\nSaved summary bar+pie figure to: {summary_fig_path}")
else:
    print("\nNo successful subjects -- skipping summary figure.")


print("\n" + "=" * 80)
print("BATCH PROCESSING COMPLETED (FINAL: RANK-AWARE ICA)")
print("=" * 80)
...
print(f"  Topography plots saved (for visual review): {topo_plots_saved} -> {DIR_TOPO}")

print(f"\nBandpass filter used: {LOW_FREQ}-{HIGH_FREQ} Hz")
print(f"ICA component selection: rank-aware (per subject)")
